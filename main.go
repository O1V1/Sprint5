...
type InfoMessage struct {
	...
	Distance     float64       // км расстояние, которое преодолел пользователь
	Speed        float64       // км/ч средняя скорость, с которой двигался пользователь
...
func (w Walking) Calories() float64 {
	res := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/w.Height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return res
}
